// Estrutura completa da Bíblia com capítulos e versículos
// Fonte: Bíblia Sagrada (Almeida Corrigida Fiel)
...
export const getBookList = () => {
  return Object.entries(bibleBooks).map(([key, value]) => ({
    value: key,
    label: value.name,
...
export const getChapters = (bookKey) => {
  if (!bookKey || !bibleBooks[bookKey]) return [];
  const chapters = bibleBooks[bookKey].chapters;
  return Object.keys(chapters).map(ch => parseInt(ch));
};

// Helper function to get verses for a chapter
export const getVerses = (bookKey, chapter) => {
  if (!bookKey || !chapter || !bibleBooks[bookKey]) return [];
  const verseCount = bibleBooks[bookKey].chapters[chapter];
  if (!verseCount) return [];
  return Array.from({ length: verseCount }, (_, i) => i + 1);
};

// Helper function to format reference
export const formatReference = (bookKey, chapter, verseStart, verseEnd) => {
  if (!bookKey || !chapter) return '';
  const bookName = bibleBooks[bookKey]?.name || '';
  let ref = `${bookName} ${chapter}:${verseStart || ''}`;
